  ...
  try {
    ...
    configs.forEach((config, index) => {
      ...
      const inputs = config.querySelectorAll(
        'input[type="number"]:not(.config-name)'
      )
...
function addConfiguration (config = {}) {
  ...
  const defaultEmptyD10 =
    config.d10 === null || config.d10 === undefined ? 1 : config.d10
            ...
            <div class="config-header">
                ...
                <div>
                    ...
                    <select class="line-style" style="margin-left: 10px;">
                        <option value="solid" ${
                          config.lineStyle === 'solid' ? 'selected' : ''
                        }>Solid</option>
                        <option value="dashed" ${
                          config.lineStyle === 'dashed' ? 'selected' : ''
                        }>Dashed</option>
                        <option value="dotted" ${
                          config.lineStyle === 'dotted' ? 'selected' : ''
                        }>Dotted</option>
                        <option value="dashDot" ${
                          config.lineStyle === 'dashDot' ? 'selected' : ''
                        }>Dash-Dot</option>
                    </select>
                </div>
...
function updateConfigNames () {
  ...
  configs.forEach((config, index) => {
    ...
    const diceInputs = config.querySelectorAll(
      'input[type="number"]:not(.config-name)'
    )
    const diceValues = Array.from(diceInputs).map(input => input.value)

    if (!nameInput.value) {
      nameInput.placeholder = `Config ${index + 1} (${diceValues[0]}d4/${
        diceValues[1]
      }d10/${diceValues[2]}d12)`
    }
  })
}
...
function getAllConfigurations () {
  return Array.from(document.querySelectorAll('.config-box')).map(config => {
    const numberInputs = config.querySelectorAll(
      'input[type="number"]:not(.config-name)'
    )
    return {
      name:
        config.querySelector('.config-name').value ||
        config.querySelector('.config-name').placeholder,
